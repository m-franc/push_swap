...
/*   Updated: 2017/04/27 12:42:34 by mfranc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

t_node			*ft_new_node(t_ctl **a_ctl, t_node *check, char *integer)
{
	t_node		*node;
	long long 	num;
	...
	if (!(node = (t_node*)malloc(sizeof(t_node))))
		return (NULL);
	check = FIRST((*a_ctl));
	ft_init_index(check, node);
	check = FIRST((*a_ctl));
	ft_push_back(check, node);
	node->data = num;
	node->next = NULL;
	return (node);
}
...
int				ft_arg_got_many_int(t_ctl **a_ctl, char *arg)
{
	...
	while (ints[++i])
	{
		if (!(new = ft_new_node(a_ctl, check, ints[i])))
...
int				ft_arg_got_one_int(t_ctl **a_ctl, char *arg)
{
	...
	check = FIRST((*a_ctl));
	if (!(new = ft_new_node(a_ctl, check, arg)))
